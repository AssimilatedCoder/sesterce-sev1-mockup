...
export const CalculatorTabEnhanced: React.FC<CalculatorTabEnhancedProps> = ({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            {spec.vendor === 'amd' && (
              <div className="mt-2 px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />
                AMD ROCm software stack required
              </div>
            )}
            {spec.interconnect !== 'nvlink' && config.numGPUs > 10000 && (
              <div className="mt-2 px-2 py-1 bg-orange-100 text-orange-800 rounded text-xs font-medium flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />
      ...
      {/* Multi-Vendor Architecture Considerations */}
      {(spec.vendor === 'amd' || spec.interconnect === 'pcie' || spec.warnings) && (
        <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-4 rounded-lg border border-yellow-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div>
              <h4 className="font-semibold text-yellow-800 mb-2">Architecture: {spec.vendor.toUpperCase()} {spec.architecture}</h4>
              <ul className="space-y-1 text-yellow-700">
                <li>• <strong>Interconnect:</strong> {spec.interconnect.toUpperCase()} 
                  {spec.interconnect === 'nvlink' ? ' (High bandwidth, optimal for training)' : 
                   spec.interconnect === 'pcie' ? ' (Limited multi-GPU scaling)' :
                   spec.interconnect === 'infinity-fabric' ? ' (AMD native, high performance)' : ''}
                </li>
                <li>• <strong>Recommended Network:</strong> {spec.networkingRecommendation.toUpperCase()}
                  {spec.networkingRecommendation === 'roce' ? ' (RoCE over Ethernet)' : 
                   spec.networkingRecommendation === 'infiniband' ? ' (RDMA native)' : ''}
                </li>
                <li>• <strong>Multi-tenant Optimal:</strong> {spec.multiTenantOptimal ? 'Yes' : 'No'}</li>
                <li>• <strong>Rack Configuration:</strong> {spec.rackSize} GPUs per server ({(spec.rackPower/1000).toFixed(1)}kW)</li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold text-yellow-800 mb-2">Deployment Considerations</h4>
              <ul className="space-y-1 text-yellow-700">
                {spec.vendor === 'amd' && (
                  <>
                    <li>• AMD ROCm software stack required</li>
                    <li>• Different memory model vs CUDA</li>
                    <li>• Infinity Fabric interconnect topology</li>
                  </>
                )}
                {spec.interconnect === 'pcie' && (
                  <>
                    <li>• Limited multi-GPU communication</li>
                    <li>• Network-dependent for GPU-to-GPU traffic</li>
                    <li>• Better for inference than training workloads</li>
                  </>
                )}
                {spec.vendor === 'nvidia' && spec.interconnect !== 'nvlink' && (
                  <>
                    <li>• Professional driver stack</li>
                    <li>• Optimized for multi-tenant environments</li>
                    <li>• Lower power consumption per GPU</li>
                  </>
                )}
              </ul>
            </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            ...
            {spec.networkingRecommendation && spec.networkingRecommendation !== config.fabricType && (
              <div className="mt-2 px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-medium flex items-center gap-1">
                <AlertTriangle className="w-3 h-3" />
                Recommended: {spec.networkingRecommendation === 'infiniband' ? 'InfiniBand' : 
                             spec.networkingRecommendation === 'ethernet' ? 'Ethernet RoCEv2' : 
                             spec.networkingRecommendation === 'roce' ? 'RoCE (Ethernet)' : 
                             spec.networkingRecommendation} for {spec.vendor.toUpperCase()} {spec.architecture}
              </div>
            )}
          </div>
